...
Set `BOT_OWNER_ID` to your Discord user ID for owner-only tools. `$bi client_id <bot application ID>` generates an OAuth2 install link privately for the owner; Discord still requires the user to authorize the bot and have permission to add it to a server. Bot information remains available through `$botinfo` and `$about`.
