...
module.exports = {
  ...
  async execute(interaction, client) {
    try {
      ...
      const robber = client.db.getEconomy(interaction.guildId, interaction.user.id) || { coins: 0 };
      const targetEcon = client.db.getEconomy(interaction.guildId, target.id) || { coins: 0 };
      ...
      await interaction.reply({ embeds: [embed("error", "Rob failed", error.message)] });
    }
  },
};
