...
module.exports = {
  ...
  async execute(interaction) { const seconds = interaction.options.getInteger("seconds"); await interaction.channel.setRateLimitPerUser(seconds); await interaction.reply({ embeds: [embed("moderation", "Slowmode updated", `Slowmode is now **${seconds}s**.`)] }); },
};
