...
async function execute(interaction, client, name) {
  ...
  if (name === "avatar") { const user = interaction.options.getUser("user") || interaction.user; await interaction.reply({ embeds: [embed("info", `${user.username}'s avatar`, user.displayAvatarURL({ size: 1024 }))] }); return; }
    ...
    if (name === "nuke") { const channel = interaction.channel; const clone = await channel.clone({ reason: `Nuked by ${interaction.user.tag}` }); await channel.delete(`Nuked by ${interaction.user.tag}`); await clone.send({ content: `Nuked by ${interaction.user}`, allowedMentions: { users: [interaction.user.id] } }); return; }
